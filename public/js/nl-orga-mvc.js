...
this["NL"]["Template"]["organization-party"] = function(obj) {obj || (obj = {});var __t, __p = '', __e = _.escape, __j = Array.prototype.join;function print() { __p += __j.call(arguments, '') }with (obj) {__p += '<div class="row"><div class="col-lg-12"><div class="row">'; _.each(partys, function(party, idx){ ;__p += '<div class="col-lg-3"><div class="list-group"><div class="list-group-item ';if(party.partyname == myparty){;__p += 'list-group-item-danger';}else{;__p += 'active';};__p += '"><b> ' +__e( party.partyname ) +'</b> <span class="badge">' +__e( party.members.length) +'</span></div>'; if(party.job){ ; if( Array.isArray(party.job) ){ ;	_.each(party.job, function(job) {;__p += '<div class="list-group-item list-group-item-warning"><b>' +__e( job ) +'</b></div>'; }) ; } else { ;__p += '<div class="list-group-item list-group-item-warning"><b>' +__e( party.job ) +'</b></div>'; } ; } ;__p += '<div class="list-group-item ">'; _.each(party.members,function(member){ ;__p += '<div class="player '; if ( member.username == username ) {;__p += ' bg-success ';};__p += '"><span class="' +__e( member.profession) +'-icon"></span>'; if( member.commander) { ;__p += '<span class="' +__e( member.commander) +'-icon"></span>'; } ; if( member.havebanner == true ) { ;__p += '<span class="banner-icon"></span>'; } ; if( member.havefood == true ) { ;__p += '<span class="food-icon"></span>'; } ;__p += '<span class="user">' +__e( member.displayname != '' ? member.displayname : member.username ) +'</span></div>';});;__p += '</div><div class="list-group-item list-group-item-info text-center"><span>/join ' +__e(party.members[0].username ) +'</span></div></div></div>'; if ( (idx % 4) == 3 ) {;__p += '</div><div class="row">';};});;__p += '</div></div></div>';/*  function SelectText(element) {var doc = document, text = doc.getElementById(element), range, selection;if (doc.body.createTextRange) {range = document.body.createTextRange();range.moveToElementText(text);range.select();} else if (window.getSelection) {selection = window.getSelection();range = document.createRange();range.selectNodeContents(text);selection.removeAllRanges();selection.addRange(range);}}document.onclick = function(e) {if (e.target.className === 'click') {SelectText('selectme');}};*/;}return __p};
